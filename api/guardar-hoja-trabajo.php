...
$articulos       = isset($body['articulos'])   && is_array($body['articulos'])
                   ? $body['articulos']
                   : [];
$procedimientos  = isset($body['procedimientos']) && is_array($body['procedimientos'])
                   ? $body['procedimientos']
                   : [];

// Campos de resultado e indicaciones
$resultado       = isset($body['visitas_sitio_hoja_trabajo_resultado']) ? (string) $body['visitas_sitio_hoja_trabajo_resultado'] : '';
$indicaciones    = isset($body['visitas_sitio_hoja_trabajo_indicaciones']) ? (string) $body['visitas_sitio_hoja_trabajo_indicaciones'] : '';
...
try {
    ...
    if ($registro) {
        // Ya existe → UPDATE con los nuevos campos
        ...
        $stmt = $pdo->prepare(
            'UPDATE visitas_sitio_hoja_trabajo
                SET id_visita = :id_visita,
                    visitas_sitio_hoja_trabajo_resultado = :resultado,
                    visitas_sitio_hoja_trabajo_indicaciones = :indicaciones
        ...
        // No existe → INSERT con los nuevos campos
        $stmt = $pdo->prepare(
            'INSERT INTO visitas_sitio_hoja_trabajo 
                (id_visita, visitas_sitio_hoja_trabajo_resultado, visitas_sitio_hoja_trabajo_indicaciones)
             VALUES (:id_visita, :resultado, :indicaciones)'
        );
        $stmt->execute([
            ':id_visita' => $id_visita,
            ':resultado' => $resultado,
    ...
    // ── Paso 5: Eliminar procedimientos previos ───────────────────────────────
    $stmt = $pdo->prepare(
        'DELETE FROM visitas_sitio_hoja_trabajo_procedimiento
          WHERE visitas_sitio_hoja_trabajo_id = :hoja_id'
    );
    $stmt->execute([':hoja_id' => $visitas_sitio_hoja_trabajo_id]);

    // ── Paso 6: Insertar artículos actuales ───────────────────────────────────
    if (!empty($articulos)) {
        $stmt = $pdo->prepare(
    ...
    // ── Paso 7: Insertar procedimientos actuales ─────────────────────────────
    if (!empty($procedimientos)) {
        $stmt = $pdo->prepare(
            'INSERT INTO visitas_sitio_hoja_trabajo_procedimiento
                (visitas_sitio_hoja_trabajo_id,
                 visitas_procedimiento_id,
                 visitas_sitio_hoja_trabajo_procedimiento_comentario)
             VALUES
                (:hoja_id, :id_procedimiento, :comentario)'
        );

        foreach ($procedimientos as $proc) {
            $id_procedimiento = isset($proc['id']) ? (int) $proc['id'] : 0;
            $comentario       = isset($proc['comentario']) ? (string) $proc['comentario'] : '';

            if ($id_procedimiento <= 0) {
                throw new \InvalidArgumentException(
                    "Procedimiento con id inválido."
                );
            }
