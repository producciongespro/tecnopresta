...
try {
    ...
    $stmtTicket = $pdo->prepare("SELECT id FROM tickets_visitas_sitio WHERE id_visita = :id_visita LIMIT 1");
    $stmtTicket->execute([':id_visita' => $id_visita]);
    $filaTicket = $stmtTicket->fetch(PDO::FETCH_ASSOC);

    if (!$filaTicket) {
        echo json_encode([
            'success' => false,
            'message' => 'No se encontró el ticket asociado a la visita'
        ]);
        exit;
    }

    $id_ticket = $filaTicket['id'];
    ...
    try {
        ...
        $stmtDeleteAsignaciones = $pdo->prepare("DELETE FROM tickets_visitas_sitio WHERE id_visita = :id_visita");
    ...
    error_log("Error en modificar-visita-sitio.php: " . $e->getMessage());
    echo json_encode([
        'success' => false,
