...
<a href="navegar.php?ruta=formulario_sub_modulos.php&subsistema_id=2&modulo_id=8" class="btn-flotante-regresar" title="Volver al panel general">
...
function renderizarVisitas(visitas, estado) {
    ...
    visitas.forEach(visita => {
        ...
        if (estado === 1) {
            ...
            botonesHTML = `
                <button class="btn" style="background-color: var(--mep-light); color: #0a0a0a;"
                    data-id="${escaparHTML(visita.id_visita)}"
                    onclick="generarInformeVisitaTecnica(this.dataset.id)">
                    <i class="bi bi-file-earmark-text me-1"></i>Generar Informe Visita Técnica
                </button>
            `;
        }
        ...
        card.innerHTML = `
            <div class="card-body">
                <div class="row mb-1">
                    <div class="col-12 col-md-3 fw-bold">Fecha de visita:</div>
                    <div class="col-12 col-md-9">${escaparHTML(visita.fecha_visita)}</div>
                </div>
                <div class="row mb-1">
                    <div class="col-12 col-md-3 fw-bold">Institución:</div>
                    <div class="col-12 col-md-9">${escaparHTML(visita.codigo_institucion)} - ${escaparHTML(visita.nombre_institucion)}</div>
                </div>
                <div class="row mb-1">
                    <div class="col-12 col-md-3 fw-bold">Soportistas asignados:</div>
                    <div class="col-12 col-md-9">${escaparHTML(visita.soportistas_asignados)}</div>
                </div>
                <div class="row mb-1">
                    <div class="col-12 col-md-3 fw-bold">Orígenes presupuestarios:</div>
                    <div class="col-12 col-md-9">${escaparHTML(visita.origenes_presupuestarios)}</div>
                </div>
                <div class="row mb-1">
                    <div class="col-12 col-md-3 fw-bold">Descripción del problema:</div>
                    <div class="col-12 col-md-9">${escaparHTML(visita.descripcion_problema)}</div>
                </div>
                <div class="row">
                    <div class="col-12 mt-3 text-center d-flex gap-2 justify-content-center">
                        ${botonesHTML}
                    </div>
                </div>
            </div>
...
function generarInformeVisitaTecnica(idVisita) {
    sessionStorage.setItem('id_visita', idVisita);
    window.open('informe_visita_tecnica.html', '_blank');
}
</script>
    </body>
