...
<head>
    ...
    <title>TecnoPresta - Crear visita</title>
...
<div class="container">
    ...
    <h4 class="text-center mt-2">Asignación de Visita Técnica a un Centro Educativo</h4>
    ...
    <form id="formCrearVisita" class="card">
        <div class="card-body">
            ...
            <div class="row g-3 mt-2">
                <div class="col-12">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="notificar_centro" checked>
                        <label class="form-check-label" for="notificar_centro">Enviar Notificación al Centro Educativo</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="notificar_soportistas" checked>
...
<!-- Modal resultado creación de visita -->
...
<script>
document.addEventListener('DOMContentLoaded', inicializarCrearVisita);
...
async function inicializarCrearVisita() {
    precargarFechaHoraActual();
    ...
    document.getElementById('formCrearVisita').addEventListener('submit', guardarVisita);
}

function precargarFechaHoraActual() {
...
async function guardarVisita(event) {
    ...
    try {
        const idUsuarioRegistro = obtener_cedula();
        if (!idUsuarioRegistro) {
            mostrarModalResultado(false, 'No se pudo obtener el usuario de la sesión.');
        ...
        const response = await fetch('/api/crear-visita-manual.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.success === true) {
            mostrarModalResultado(true, data.message || 'La visita se creó satisfactoriamente');
        } else {
            mostrarModalResultado(false, data.message || 'Error al crear la visita');
            btnGuardar.disabled = false;
        }
